...
class SnakeScene {
    ...
    init(_ config: SnakeConfig, _ scene: SCNScene, _ x: Float, _ y: Float, _ z: Float) {
        self.config = config
        self.scene = scene
        self.x = x
        self.y = y
        self.z = z
        self.frame = createWellFrame(config.width, config.length)
        scene.rootNode.addChildNode(self.frame)
    }
    
    func show(_ current: SnakeState) {
...
//        }
    ...
    func addToWell(_ current: SnakeState) {
        recent?.removeFromParentNode()
//        let tetromino = current.tetromino()
//        for i in 0...3 {
...
//        }
    ...
    func drop(from: SnakeState, to: SnakeState) -> CFTimeInterval {
        // use animation time from 0.1 to 0.4 seconds depending on distance
        let delta = from.y - to.y
        let percent = Double(delta - 1) / Double(config.length - 1)
    ...
    func showGameOver(_ scores: Int) {
        ...
        let floor = createNode(SCNPlane(width: 10, height: 10), matrix, SnakeScene.floorColor)
    ...
    private func showLinesScores(_ line: Float, _ scores: Int) {
        let node = createNode(text("+\(scores)"), translate(5, line, 2).scale(0.001), SnakeScene.scoresColor)
    ...
    private func showFinalScores(_ scores: Int) {
        ...
        let y = Float(config.length / 2)
        let node = createNode(text("Scores: \(scores)"), translate(x, y).scale(0.003), SnakeScene.titleColor)
        self.scene.rootNode.addChildNode(node)
    }
    ...
    private func block(_ state: SnakeState, _ x: Int, _ y: Int) -> SCNNode {
        ...
        return createNode(box, matrix, SnakeScene.colors[state.index])
    }
    ...
    private func addLine(to node: SCNNode, _ w: Float, _ h: Float, _ l: Float, _ x: Float, _ y: Float, _ z: Float) {
        ...
        node.addChildNode(createNode(line, matrix, SnakeScene.wellColor))
    }
